...
Stella provides a simple and centralized class for handling rendering primitives directly, for things such as trails and etcetera. You do not need to worry about the behind the scenes goings on to use them.
...
Stella provides an interface to draw primitives in for NPCs and projectiles with pixelation applied to them. To use, make your ModNPC/ModProjectile implement ``IPixelatedPrimitiveRenderer`` and use the same render trail call as above, setting ``Pixelate`` to true in the ``PrimitiveSettings`` constructor.
...
The ``uWorldViewProjection`` parameter, on the other hand, is required for primitive shaders used by Stella to work, being responsible for vertex transformations. If it is misconfigured or given an incorrect name, the primitives will not render properly.
