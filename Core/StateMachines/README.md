# State Machines
Stella provides a framework for the implementation and handling of behavior states via a concept known as a pushdown automata, which can conceptually be thought of like a memory stack that holds local data about a given state.
...
timers. For convenience, Stella provides a simplified ``EntityAIState`` with a simpler integer timer.
